    ...
    db.plantcare.getConnection((err, connection) => {
        ...
        connection.beginTransaction((transactionErr) => {
            ...
            db.plantcare.query(
                fixedAssetSql,
                fixedAssetParams,
                (fixedAssetErr, fixedAssetResult) => {
                    if (fixedAssetErr) {
                    ...
                    if (category === "Building and Infrastructures") {
                        const getFarmSql = `SELECT district FROM farms WHERE id = ? LIMIT 1`;

                        db.plantcare.query(getFarmSql, [farmId], (farmErr, farmResult) => {
                            if (farmErr) {
                                return connection.rollback(() => {
                                    connection.release();
                                    return res.status(500).json({
                                        message: "Error fetching farm district",
                                        error: farmErr,
                                    });
                                });
                            }
                            ...
                            if (!farmResult || farmResult.length === 0) {
                                return connection.rollback(() => {
                                    ...
                                    return res.status(404).json({
                                        message: "Farm not found for the provided farmId.",
                                    });
                                });
                            }

                            const farmDistrict = farmResult[0].district;
                            ...
                            db.plantcare.query(
                                ...
                                (buildingErr, buildingResult) => {
                                    if (buildingErr) {
                                        return connection.rollback(() => {
                                            connection.release();
                                            return res.status(500).json({
                                                message:
                                                    "Error inserting into buildingfixedasset table",
                                                error: buildingErr,
                                            });
                                        });
                                    }
                                    ...
                                    switch (ownership) {
                                        ...
                                        case "Leased Building":
                                            ownershipSql = `INSERT INTO ownershipleastfixedasset (buildingAssetId, startDate, durationYears, durationMonths, leastAmountAnnually) VALUES (?, ?, ?, ?, ?)`;
                                            ownershipParams = [
                                                buildingAssetId,
                                                formattedStartDate,
                                                durationYears,
                                        ...
                                        default:
                                            return connection.rollback(() => {
                                                ...
                                                return res.status(400).json({
                                                    message:
                                                        "Invalid ownership type provided for building asset.",
                                                });
                                            });
                                    }
                                        ...
                                        (ownershipErr) => {
                                            if (ownershipErr) {
                                                return connection.rollback(() => {
                                                    connection.release();
                                                    return res.status(500).json({
                                                        message: "Error inserting into ownership table",
                                                        error: ownershipErr,
                                                    });
                                                });
                                            }
                                            ...
                                            connection.commit((commitErr) => {
                                                ...
                                                return res.status(201).json({
                                                    message:
                                                        "Building fixed asset with ownership created successfully.",
                                                });
                                            });
                                        },
                                    );
                        ...
                        // Fetch district from farms table using farmId
                        const getFarmSql = `SELECT district FROM farms WHERE id = ? LIMIT 1`;
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
                            ...
                            db.plantcare.query(
                                ...
                                [
                                    ...
                                    extentp,
                                    landownership,
                                    farmDistrict, // ← now from farms table
                                    landFenced,
                                    perennialCrop,
                                ],
                                (landErr, landResult) => {
                                   ...
                                   VALUES (?, ?, ?)`;
                                        db.plantcare.query(
                                            ownershipOwnerSql,
                                            [landAssetId, formattedIssuedDate, estimateValue],
                                            (ownershipErr) => {
                                                if (ownershipErr) {
                                                    return connection.rollback(() => {
                                                        connection.release();
                                                        return res.status(500).json({
                                                            message:
                                                                "Error inserting into ownershipownerfixedasset table",
                                                            error: ownershipErr,
                                                        });
                                                    });
                                                }
                                                connection.commit((commitErr) => {
                                                    ...
                                                    return res.status(201).json({
                                                        message:
                                                            "Land fixed asset with ownership created successfully.",
                                                    });
                                                });
                                            },
                                        );
                                   ...
                                   VALUES (?, ?, ?, ?, ?)`;
                                            ...
                                            (leaseErr) => {
                                                if (leaseErr) {
                                                    return connection.rollback(() => {
                                                        ...
                                                        return res.status(500).json({
                                                            message:
                                                                "Error inserting into ownershipleastfixedasset table",
                                                            error: leaseErr,
                                                        });
                                                    });
                                                }
                                                connection.commit((commitErr) => {
                                                    ...
                                                    return res.status(201).json({
                                                        message:
                                                            "Land fixed asset with lease ownership created successfully.",
                                                    });
                                                });
                                            },
                                        );
                                    ...
                                    VALUES (?, ?, ?)`;
                                        db.plantcare.query(
                                            ownershipPermitSql,
                                            [landAssetId, formattedIssuedDate, permitFeeAnnually],
                                            (permitErr) => {
                                                if (permitErr) {
                                                    return connection.rollback(() => {
                                                        connection.release();
                                                        return res.status(500).json({
                                                            message:
                                                                "Error inserting into ownershippermitfixedasset table",
                                                            error: permitErr,
                                                        });
                                                    });
                                                }
                                                connection.commit((commitErr) => {
                                                    ...
                                                    return res.status(201).json({
                                                        message:
                                                            "Land fixed asset with permit ownership created successfully.",
                                                    });
                                                });
                                            },
                                        );
                                    ...
                                    VALUES (?, ?)`;
                                            ...
                                            (sharedErr) => {
                                                if (sharedErr) {
                                                    return connection.rollback(() => {
                                                        ...
                                                        return res.status(500).json({
                                                            message:
                                                                "Error inserting into ownershipsharedfixedasset table",
                                                            error: sharedErr,
                                                        });
                                                    });
                                                }
                                                connection.commit((commitErr) => {
                                                    ...
                                                    return res.status(201).json({
                                                        message:
                                                            "Land fixed asset with shared ownership created successfully.",
                                                    });
                                                });
                                            },
                                        );
                                        ...
                                        return connection.rollback(() => {
                                            ...
                                            return res.status(400).json({
                                                message:
                                                    "Invalid ownership type provided for land asset.",
                                            });
                                        });
                                    }
                                },
                        ...
                        db.plantcare.query(
                            ...
                            (machToolsErr, machToolsResult) => {
                                if (machToolsErr) {
                                    return connection.rollback(() => {
                                        connection.release();
                                        return res.status(500).json({
                                            message: "Error inserting into machtoolsfixedasset table",
                                            error: machToolsErr,
                                        });
                                    });
                                }
                                        ...
                                        (warrantyErr) => {
                                            if (warrantyErr) {
                                                return connection.rollback(() => {
                                                    ...
                                                    return res.status(500).json({
                                                        message:
                                                            "Error inserting into machtoolsfixedassetwarranty table",
                                                        error: warrantyErr,
                                                    });
                                                });
                                            }
                        ...
                        return connection.rollback(() => {
                            ...
                            return res
                                .status(400)
                                .json({ message: "Invalid category provided." });
                        });
                    }
                },
            );
                    ...
                    (ownershipErr, ownershipResults) => {
                        if (ownershipErr) {
                            return connection.rollback(() => {
                                connection.release();
                                return res.status(500).json({
                                    message: "Error retrieving ownership details",
                                    error: ownershipErr,
                                });
                            });
                        }
                ...
                db.plantcare.query(updateAssetQuery, updateParams, (queryErr) => {
                    if (queryErr) {
                        ...
                        return connection.rollback(() =>
                            res
                                .status(500)
                                .json({ message: "Error updating asset", error: queryErr }),
                        );
                    }
                        ...
                        if (ownership !== oldOwnership) {
                            ...
                            `);
                                insertParams.push([
                                    ...
                                    ownershipDetails.durationYears !== undefined
                                        ? String(ownershipDetails.durationYears)
                                        : "0",
                                    ownershipDetails.durationMonths !== undefined
                                        ? String(ownershipDetails.durationMonths)
                                        : "0",
                            ...
                            if (ownership === "Own") {
                                ...
                                const formattedIssuedDate = formatDateToMySQLDateOnly(
                            ...
                            executeUpdateQueries(
                                res,
                                assetId,
                                ownershipUpdateQueries,
                                ownershipUpdateParams,
                                updateAssetQuery,
                                updateParams,
                            );
                        }
                    ...
                    updateFixedAssetUpdatedBy((trackErr) => {
                        ...
                        db.plantcare.query(warrantyQuery, warrantyParams, (warrantyErr) => {
                            if (warrantyErr) {
                                ...
                                return connection.rollback(() =>
                                    res.status(500).json({
                                        message: "Error updating warranty details",
                                        error: warrantyErr,
                                    }),
                                );
                            }
                            ...
                            connection.commit((commitErr) => {
                                ...
                                connection.release();
                                return res.status(200).json({
                                    message: "Asset and ownership details updated successfully.",
                                });
                            });
                        });
                    });
...
function executeDeleteAndInsertQueries(
    res,
    deleteQueries,
    deleteParams,
    insertQueries,
    ...
    db.plantcare.getConnection((err, connection) => {
        if (err) {
            ...
            return res
                .status(500)
                .json({ message: "Error acquiring connection", error: err });
        }
        ...
        connection.beginTransaction((transErr) => {
            if (transErr) {
                ...
                return res
                    .status(500)
                    .json({ message: "Error starting transaction", error: transErr });
            }

            // Create a promise for delete operations
            let deletePromises = deleteQueries.map(
                (query) =>
            ...
            Promise.all(deletePromises)
                .then(() => {
                    ...
                    let insertPromises = insertQueries.map(
                        (query, i) =>
                            new Promise((resolve, reject) => {
                ...
                .catch((err) => {
                    ...
                    connection.rollback(() => {
                        connection.release();
                        return res.status(500).json({
                            message: "Error executing ownership change",
                            error: err,
                        });
                    });
                });
...
async function executeUpdateQueries(
    res,
    assetId,
    updateQueries = [],
    updateParams = [],
        ...
        connection.beginTransaction(async (transErr) => {
            ...
            if (!Array.isArray(updateQueries) || !Array.isArray(updateParams)) {
                return connection.rollback(() => {
                    ...
                    return res
                        .status(500)
                        .json({ message: "Invalid parameters for update queries" });
                });
            }

            // Create a promise for update operations
            let updatePromises = updateQueries.map(
                (query, i) =>
                    new Promise((resolve, reject) => {
            ...
            await Promise.all(updatePromises)
                ...
                .catch((err) => {
                    ...
                    connection.rollback(() => {
                        connection.release();
                        return res.status(500).json({
                            message: "Error executing ownership update",
                            error: err,
                        });
                    });
                });
        ...
        db.plantcare.query(query, params, (err) => {
            if (err) {
                return db.plantcare.rollback(() =>
                    res
                        .status(500)
                        .json({ message: "Ownership update error", error: err }),
                );
            }
            executeNext(i + 1);
        });
    ...
    db.plantcare.getConnection((err, connection) => {
        ...
        connection.beginTransaction((transErr) => {
            if (transErr) {
                ...
                return res
                    .status(500)
                    .json({ message: "Transaction error", error: transErr });
            }
            ...
            Promise.all(ownershipDeletionPromises)
                    ...
                    connection.query(deleteAssetQuery, [userId, assetId], (queryErr) => {
                        if (queryErr) {
                            return connection.rollback(() => {
                                connection.release();
                                return res.status(500).json({
                                    message: "Error deleting asset details",
                                    error: queryErr,
                                });
                            });
                        }
                        ...
                        connection.commit((commitErr) => {
                            ...
                            connection.release();
                            return res.status(200).json({
                                message: "Asset and ownership details deleted successfully.",
                            });
                        });
                    });
                })
                .catch((ownershipDeletionErr) => {
                    ...
                    connection.rollback(() => {
                        connection.release();
                        return res.status(500).json({
                            message: "Error deleting ownership details",
                            error: ownershipDeletionErr,
                        });
                    });
                });
        });
