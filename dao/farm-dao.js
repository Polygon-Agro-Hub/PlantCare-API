...
exports.createFarmWithStaff = async (farmData) => {
    ...
    try {
        ...
        const [farmResult] = await connection
            .promise()
...
function generateRegCode(nic, farmId, farmIndex) {
    ...
    if (nicDigits.length === 10) {
        nicPart = nicDigits.substring(6, 9);
    } else if (nicDigits.length === 12) {
        nicPart = nicDigits.substring(9, 12);
...
exports.createPaymentAndUpdateMembership = async (paymentData) => {
    ...
    try {
        ...
        const [existingPayments] = await connection
            .promise()
...
exports.getOngoingCultivationsByUserIdAndFarmId = (
    userId,
    farmId,
    callback,
) => {
...
exports.checkCropCountByFarm = (cultivationId, farmId) => {
    const sql =
        "SELECT COUNT(id) as count FROM ongoingcultivationscrops WHERE ongoingCultivationId = ? AND farmId = ?";
    return query(sql, [cultivationId, farmId]);
};

exports.checkEnrollCropByFarm = (cultivationId, farmId) => {
    const sql =
        "SELECT cropCalendar, id FROM ongoingcultivationscrops WHERE ongoingCultivationId = ? AND farmId = ?";
    return query(sql, [cultivationId, farmId]);
};
...
exports.enrollOngoingCultivationCrop = (
    ...
    const sql =
        "INSERT INTO ongoingcultivationscrops(ongoingCultivationId, cropCalendar, farmId, extentha, extentac, extentp, startedAt, cultivationIndex) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    return query(sql, [
        cultivationId,
        cropId,
        farmId,
...
exports.updateFarm = async (farmData) => {
    ...
    try {
        ...
        const [updateResult] = await connection
            .promise()
            .query(updateFarmSql, farmValues);

        if (updateResult.affectedRows === 0) {
            throw new Error(
                "No farm was updated. Please check if the farm exists and you have permission to update it.",
            );
        }
...
exports.deleteFarm = (farmId) => {
    return new Promise((resolve, reject) => {
        ...
        db.plantcare.query(checkSql, [farmId], (err, checkResult) => {
            ...
            `;
            db.plantcare.query(
                deleteSlaveCropCalendarSql,
                [farmId],
                (err, slaveCropDeleteResult) => {
                    if (err) {
                    ...
                    db.plantcare.query(
                        ...
                        (err, farmStaffDeleteResult) => {
                            ...
                            db.plantcare.query(
                                deleteCurrentAssetsSql,
                                [farmId],
                                (err, currentAssetDeleteResult) => {
                                    if (err) {
                                    ...
                                    db.plantcare.query(
                                        deleteFixedAssetsSql,
                                        [farmId],
                                        (err, fixedAssetDeleteResult) => {
                                            if (err) {
                                                console.error(
                                                    "Error deleting fixedasset records:",
                                                    err,
                                                );
                                                reject(err);
                                                return;
...
exports.getAssetsByCategory = (userId, category, farmId) => {
    return new Promise((resolve, reject) => {
        ...
        if (Array.isArray(category)) {
            ...
            query =
                "SELECT * FROM currentasset WHERE userId = ? AND farmId = ? AND category = ?";
            values = [userId, farmId, category];
        }
...
exports.getAllCurrentAssets = (userId, farmId) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        const itemsSql = `
            ...
            FROM currentasset 
            WHERE userId = ? AND farmId = ?
            ORDER BY category ASC, asset ASC, brand ASC, batchNum ASC, createdAt DESC
        `;
        db.plantcare.query(
            categorySql,
            [userId, farmId],
    ...
    return new Promise((resolve, reject) => {
        const query = `
            SELECT 
                ...
                f.extentac,
                f.extentp,
                -- Total farm extent in perches
                (f.extentha * 160 + f.extentac * 4 + f.extentp) as totalExtentPerches,
                -- Already cultivated extent in perches
                COALESCE(SUM(occ.extentha * 160 + occ.extentac * 4 + occ.extentp), 0) as cultivatedExtentPerches,
                -- Available extent in perches
                (f.extentha * 160 + f.extentac * 4 + f.extentp) - 
                COALESCE(SUM(occ.extentha * 160 + occ.extentac * 4 + occ.extentp), 0) as availableExtentPerches
            FROM farms f
            LEFT JOIN ongoingcultivationscrops occ ON f.id = occ.farmId AND occ.isBlock = 0
            WHERE f.id = ?
            GROUP BY f.id, f.farmName, f.extentha, f.extentac, f.extentp
        `;
        ...
        db.plantcare.query(query, [farmId], (error, results) => {
            if (error) {
                ...
                if (results && results.length > 0) {
                    ...
                    const cultivatedAc = Math.floor(remainingCultivated / 4);
                    const cultivatedP = remainingCultivated % 4;

                    farm.cultivatedExtentha = cultivatedHa;
                    farm.cultivatedExtentac = cultivatedAc;
