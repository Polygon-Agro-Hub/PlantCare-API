...
exports.OngoingCultivaionGetById = asyncHandler(async (req, res) => {
    try {
        const { error } = ongoingCultivationSchema.validate(req.query);
...
exports.enroll = asyncHandler(async (req, res) => {
    try {
        ...
        const ongoingCultivationResult =
            await cropDao.checkOngoingCultivation(userId);

        if (!ongoingCultivationResult[0]) {
            const newCultivationResult =
                await cropDao.createOngoingCultivation(userId);
        ...
        await cropDao.enrollOngoingCultivationCrop(
            cultivationId,
            cropId,
            extentha,
            extentac,
        ...
        const enroledoncultivationcrop =
            await cropDao.getEnrollOngoingCultivationCrop(cropId, userId);
        ...
        const responseenrollSlaveCrop = await cropDao.enrollSlaveCrop(
            userId,
            cropId,
            startDate,
            onCulscropID,
...
exports.UpdateOngoingCultivationScrops = asyncHandler(async (req, res) => {
    try {
        ...
        if (!extentha || !extentac || !extentp) {
            return res
                .status(400)
                .json({ message: "Extent and Start Date are required." });
        }

        const results = await cropDao.updateOngoingCultivationCrop(
            onCulscropID,
            extentha,
            extentac,
            extentp,
        ...
        if (results.affectedRows === 0) {
            return res
                .status(404)
                .json({
                    message: "Ongoing cultivation crop not found or not updated.",
                });
        }
        res
            .status(200)
            .json({
                message:
                    "Ongoing cultivation crop and slavecropcalendardays updated successfully.",
                results,
            });
    } catch (err) {
        ...
        res
            .status(500)
            .json({
                message: "Server error. Unable to update ongoing cultivation crop.",
            });
    }
});

exports.getSlaveCropCalendarDaysByUserAndCrop = asyncHandler(
    async (req, res) => {
        try {
            ...
            const results = await cropDao.getSlaveCropCalendarDaysByUserAndCrop(
                userId,
                cropCalendarId,
                farmId,
            );
            ...
            if (results.length === 0) {
                return res.status(404).json({
                    message:
                        "No records found for the given userId, cropCalendarId, and farmId.",
                });
            }
...
exports.getSlaveCropCalendarPrgress = asyncHandler(async (req, res) => {
    try {
        ...
        const results = await cropDao.getSlaveCropCalendarPrgress(
            userId,
            cropCalendarId,
            farmId,
        );
    ...
    try {
        ...
        if (currentStatus === "completed" && status === "pending") {
            const timeDiffInHours =
                Math.abs(currentTime - new Date(createdAt)) / 36e5;
        ...
        const updateResults = await cropDao.updateTaskStatus(
            id,
            status,
            userId,
            ownerId,
...
exports.addGeoLocation = asyncHandler(async (req, res) => {
    try {
        ...
        const results = await cropDao.addGeoLocation(
            longitude,
            latitude,
            onCulscropID,
        );
